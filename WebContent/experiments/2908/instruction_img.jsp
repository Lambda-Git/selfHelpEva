	...
	<div id="mid" style="width: 80%;height: 90%;font-size:14px; ">
		<div id="textArea" style="height:70%;width:100%;">
		    ...
		    <div style="height: 70%;width: 100%;display: flex;align-items:center;justify-content:center;">
				<img id="targetpic" hidden="true"  src="z1.png" style=" align: center;max-height:90%" />
...
<script>
	var index = 0;
    var words = new Array ("您所在的环境是否是安静的？需要在安静的环境中完成测验。","您是否一个人或者只有一位监护人陪同您进行测验？","您是否可以做到一个人独立完成测验？需要独立完成,保证数据真实性。","监护人要做到不干扰不提示不暗示测试者答题,只能在答题前讲解指导语。","这是一个1分钟的练习，分2部分；请根据指导语提示，认真完成练习。","第一部分：判断当前出现的位置与先于当前位置2个出现的位置是否相同，如果相同，请点击“相同”按钮，如果不同，请点击“不同”按钮。","样例展示","第二部分：判断当前出现的位置与先于当前位置3个出现的位置是否相同，如果相同，请点击“相同”按钮，如果不同，请点击“不同”按钮。");
