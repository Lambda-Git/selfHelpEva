...
System.out.println("asdfasdfasfdasfdasf");

Users u=(Users)request.getSession().getAttribute("cu");
int uid=u.getUserid();
int taskid=Integer.parseInt(request.getParameter("taskid"));

int codematerial=Integer.parseInt(request.getParameter("codematerial"));
...
<body onmousewheel="return false;">
	<div style="position: absolute;top:20px;right:20px;padding:10px;font-size:14px;color:red">时间:<span  id="remain_time">240</span></div>
	...
	<div style="position: absolute;top:140px;left:15px;padding:10px;height:300px;font-size:12px;width:28px;background: #eee">剩余时间<div id="timebar" style="height:280px;width:28px;background:green"></div></div>
	<div style="position: absolute;top:47%;right:47%;width:150px;height:150px;z-index:9999999;vertical-align:middle;text-align: center;">
	<img id="face" src="./img/correct.png" width='150px' height='150px' style="display:none;vertical-align:middle;text-align: center;"> </img>
...
//-----【全局变量声明】
var all_time=60,delay=4000,q_now=0,num1,num2,score=0,wrong_num=0;
var q_lib=[],q_answer=[];
var data={};
data.stimset='';
...
function all_time_go(){
	if (all_time == 0) {
		...
		$("#remain_time").html(all_time);
		all_time--;
		var height_tmp=parseInt(all_time/60*280);
...
function gen_question(){
	...
	for(var i=0;i<10;i++){
		...
		if(q_num>2){
			...
			if( (tmp[0]==num1&&tmp[1]==num2)||(tmp[1]==num1&&tmp[0]==num2)  ){q_num++;answer.push(i);}
		}
		question[i]=tmp;
...
function show_face(x){
	if(x==1){
    $("#face").attr("src",'./img/correct.png')
	}else(
	$("#face").attr("src",'./img/wrong.png')
		)
	$("#face").show(150);
    setTimeout("$('#face').hide(150)",800);
}
